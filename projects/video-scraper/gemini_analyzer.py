...
```json
{
  ...
  "full_breakdown": "逐3-5秒的详细拆解（纯文本，时间段 → 画面 → 口播/文字 → BGM/音效 → 情绪 → 营销功能）",
  "timeline": [
    {
      "time": "00:00-00:03",
      "scene": "色调|物理空间",
      ...
      "subtitle": "屏幕文字/字幕",
      "visual_desc": "画面描述",
      "camera": "半身/怼脸直拍/全身/半身+产品特写/怼脸直拍+产品特写/产品特写/运镜/其他",
      "emotion": "平稳/兴奋/紧迫/悬念/感动/证实/幽默/愤怒/其他",
      "persuasion": ["FOMO"],
      "marketing_function": "Attention(吸引注意)/Interest(引发兴趣)/Desire(激发欲望)/Action(促成行动)/Trust(建立信任)/Problem(揭示痛点)/Solution(提供方案)/其他"
    }
  ]
}
```
...
要求：
1. timeline 数组按每3-5秒一段拆解，覆盖整个视频
2. 评分要客观，找出真正的亮点和短板
3. 可复用元素要具体，不要笼统
4. 只输出 JSON，不要其他文字"""
